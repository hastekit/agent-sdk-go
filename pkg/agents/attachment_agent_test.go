...
// or writes it to conversation history.
func TestAttachmentMiddlewareAgentGeneratedImageKeepsOnlyReference(t *testing.T) {
	store := middlewareStore(t)
	persistence := history.NewInMemoryConversationPersistence()
	manager := history.NewConversationManager(persistence)
	raw := "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a6xkAAAAASUVORK5CYII="
	first := &responses.Response{Output: []responses.OutputMessageUnion{
		{OfImageGenerationCall: &responses.ImageGenerationCallMessage{
			ID: "ig_local", Status: "completed", OutputFormat: "png", Quality: "high", Size: "1024x1024", Result: raw,
		}},
		{OfFunctionCall: &responses.FunctionCallMessage{ID: "fc_1", CallID: "call_1", Name: "continue", Arguments: `{}`}},
	}}
	...
	requireStatus(t, out, agentstate.RunStatusCompleted)
	require.GreaterOrEqual(t, len(out.Output), 1)
	result := out.Output[0].OfImageGenerationCall.Result
	require.True(t, attachments.IsFileID(result))
	require.Equal(t, "high", out.Output[0].OfImageGenerationCall.Quality)
	serializedOutput, err := json.Marshal(out)
	require.NoError(t, err)
