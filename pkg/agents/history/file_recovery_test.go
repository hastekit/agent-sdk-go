...
func TestRecoveryRemainsAppendableAcrossRestarts(t *testing.T) {
	for _, suffix := range []string{"partial", "missing-newline"} {
		t.Run(suffix, func(t *testing.T) {
			...
			require.NoError(t, p.SaveMessages(ctx, "ns", "default", "run1", "", "thread", "conv", []Message{{ID: "first"}}, nil))
			...
			p, err = NewFileConversationPersistence(dir)
			require.NoError(t, err)
			require.NoError(t, p.SaveMessages(ctx, "ns", "default", "run2", "run1", "thread", "conv", []Message{{ID: "second"}}, nil))
			require.NoError(t, p.Close())
			p, err = NewFileConversationPersistence(dir)
			require.NoError(t, err)
			defer p.Close()
