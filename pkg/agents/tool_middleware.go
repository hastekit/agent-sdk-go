...
// serializeTool is SerializeTool for the executor, which also knows the
// model-facing name the loop resolved the call by.
func serializeTool(exec ExecutableToolCall) *BaseTool {
	var descriptor *BaseTool
	if exec.Tool != nil {
		descriptor = exec.Tool.GetToolDescriptor()
	}
	return serializeDescriptor(descriptor, exec.ToolName, exec.ToolCall)
}
...
// else a middleware could portably be handed. descriptor is what the tool's
// GetToolDescriptor reports, call the call it is being asked to run.
//
// It is what every path hands a middleware, so a middleware sees the same tool
// wherever the chain runs: the local executor calls it, and a runtime adapter
// that runs the chain inside its own activity or step calls it with the real
// tool's descriptor there. Name is always filled in — only a tool with a name
// of its own sets it, an MCP server's does and a function tool does not, and
// a middleware should not have to know which — and the result is never nil.
//
...
func SerializeTool(descriptor *BaseTool, call *ToolCall) *BaseTool {
	return serializeDescriptor(descriptor, "", call)
}

func serializeDescriptor(descriptor *BaseTool, name string, call *ToolCall) *BaseTool {
	if descriptor != nil && descriptor.ToolUnion.OfFunction != nil {
		if descriptor.Name != "" {
			return descriptor
		}

		// Fill the name in on a copy: GetToolDescriptor may well have handed
		// back the tool's own embedded BaseTool, which is not ours to write to.
		named := *descriptor
		named.Name = named.ToolUnion.OfFunction.Name
		return &named
	}

	if name == "" && call != nil && call.FunctionCallMessage != nil {
		name = call.Name
	}
