...
// ModelCallFailedErrorCode marks an LLM step failure Restate must not retry: a
// retry or fallback policy that ran to its end, a stream that had already
// reached the client, or a middleware's own error, which would only be refused
// again — see agents.IsTerminalModelCallError. Untyped for the same reason
// ToolCancelledErrorCode is.
const ModelCallFailedErrorCode = 597
...
func (l *RestateLLM) invoke(ctx context.Context, call *agents.ModelCall, in *responses.Request, cb func(chunk *responses.ResponseChunk)) (*responses.Response, error) {
	...
	resp, err := agents.ExecuteModelCallWithMiddleware(runCtx, l.middlewares, call, in, func(ctx context.Context, call *agents.ModelCall, in *responses.Request) (*responses.Response, error) {
	...
	if err != nil {
		...
		// middleware's own refusal must not be restarted by step retries.
		// Stops retain their own error code; a provider's failure stays
		// retryable, as any step failure is.
		if !stoppedWork(err) && agents.IsTerminalModelCallError(err) {
			return nil, restate.TerminalError(err, ModelCallFailedErrorCode)
		}
