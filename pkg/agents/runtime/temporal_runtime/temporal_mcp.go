...
func (t *TemporalMCPServer) ExecuteTool(ctx context.Context, tool *agents.BaseTool, params *agents.ToolCall, runContext map[string]any) (*agents.ToolCallResponse, error) {
	...
	resp, err := agents.ExecuteWithTrace(ctx, nil, params, func(ctx context.Context, call *agents.ToolCall) (*agents.ToolCallResponse, error) {
		return agents.ExecuteToolCallWithMiddleware(ctx, t.middlewares, agents.SerializeTool(tool, call), call, func(ctx context.Context, call *agents.ToolCall) (*agents.ToolCallResponse, error) {
...
// NewTemporalMCPProxy keeps the server name separate from its registered activity prefix.
func NewTemporalMCPProxy(workflowCtx workflow.Context, name, activityPrefix string) *TemporalMCPProxy {
