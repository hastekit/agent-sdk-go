...
// LoadTranscriptPage retains native pagination through middleware wrappers.
func (p *historyMiddlewarePersistence) LoadTranscriptPage(ctx context.Context, namespace, threadID string, opts history.TranscriptPageOptions) (*history.TranscriptPage, error) {
	return history.LoadTranscriptPage(ctx, p.ConversationPersistenceAdapter, namespace, threadID, opts)
}
