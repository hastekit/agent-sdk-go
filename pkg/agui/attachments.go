...
// Media sources carry attachment:// file references (or legacy /attachments/ URLs), never inline data.
...
// attachmentSourceRef accepts the canonical uploaded file ID and older clients'
// download URLs. Both resolve to the same application-owned immutable file.
func attachmentSourceRef(value string) (attachments.Ref, error) {
	if attachments.IsFileID(value) {
		return attachments.RefFromFileID(value)
	}
...
func validateMessageAttachments(ctx context.Context, namespace string, msgs []Message, store attachments.Store) error {
	for _, m := range msgs {
		...
		for i := range m.ContentParts {
			...
			ref, err := attachmentSourceRef(p.Source.Value)
			if err != nil {
				return err
			}
			...
			if p.Type == "image" && !strings.HasPrefix(d.MediaType, "image/") {
				return attachments.ErrInvalid
			}
			// Pin the authorized immutable version; never trust client MIME/metadata.
			ref.Version = d.Version
			p.Source = &ContentSource{Type: "url", Value: attachments.FileID(ref), MimeType: d.MediaType}
