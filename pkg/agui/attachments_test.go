...
func TestUploadedFileIDAndLegacyURLProduceSameAgentInput(t *testing.T) {
	...
	require.NoError(t, err)
	defer store.Close()
	ref, err := store.Put(t.Context(), "alice", attachments.Upload{Filename: "report.pdf", MediaType: "application/pdf", Content: strings.NewReader("%PDF-1.7\nexample\n%%EOF")})
	require.NoError(t, err)
	for _, value := range []string{attachments.FileID(ref), attachments.URL(ref)} {
		...
		require.NoError(t, validateMessageAttachments(t.Context(), "alice", []Message{msg}, store))
		...
		require.Error(t, validateMessageAttachments(t.Context(), "bob", []Message{msg}, store))
	}
}
