...
func TestChatAttachmentHTTPToProviderAndHistory(t *testing.T) {
	...
	registeredAgent := MustNewAgent(&AgentConfig{
	...
	require.NoError(t, registry.Register(registeredAgent))
	server := httptest.NewServer(web.Handler(registry, agui.WithAttachmentStore(store)))
	defer server.Close()
	...
	files := []struct {
		...
		{"document", "report.docx", []byte("PK\x03\x04original document bytes")},
		{"document", "data.csv", []byte("name,value\nalice,123\n")},
		{"document", "notes.txt", []byte("Original notes")},
		{"document", "sound.wav", []byte("RIFF\x04\x00\x00\x00WAVE")},
	}
	for _, tc := range files {
		...
		parts = append(parts, agui.ContentPart{Type: tc.kind, Source: &agui.ContentSource{Type: "url", Value: file.FileID}})
	}
	...
	for _, body := range captured {
		...
		for _, file := range files {
			require.Contains(t, body, base64.StdEncoding.EncodeToString(file.data), "original bytes for %s", file.name)
		}
		require.Contains(t, body, "data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,")
		require.Contains(t, body, "data:text/csv;base64,")
		require.NotContains(t, body, "attachment://")
		require.NotContains(t, body, "/attachments/")
	}
	require.EqualValues(t, len(files), cacheStore.opens.Load(), "one storage read per file across both turns")
	...
	require.Len(t, restored.Messages[0].ContentParts, len(files))
	...
	agent, _ := registry.Agent("attachment-http-test")
